...
import {
    ScheduleProModel,
    UpdateScheduleProModel,
} from "@/api/models/Pro-models";
...
export default function DisplaySchedules() {
    ...
    const [selectedSchedule, setSelectedSchedule] =
        useState<ScheduleProModel | null>(null);
    ...
    if (isError || !data)
        return <p className="text-red-500">Erreur de chargement des horaires.</p>;

    // 🔹 Tri des jours dans l’ordre fixe
    ...
    const handleChange = (
        field: keyof UpdateScheduleProModel,
        ...
        <div className="space-y-6">
            ...
            <div className="rounded-lg border bg-card shadow-sm">
                <Table>
                    ...
                    <TableBody>
                        {sortedSchedules.map((schedule: ScheduleProModel) => (
                            <TableRow key={schedule.id}>
                                ...
                                <TableCell>
                                    {schedule.closed
                                        ? "-"
                                        : `${schedule.morning?.start}h - ${schedule.morning?.end}h`}
                                </TableCell>

                                <TableCell>
                                    {schedule.closed
                                        ? "-"
                                        : `${schedule.afternoon?.start}h - ${schedule.afternoon?.end}h`}
                                </TableCell>
                                    ...
                                    >
                                        ...
                                        {selectedSchedule?.id === schedule.id && formData && (
                                            <DialogContent className="max-w-md">
                                                <DialogHeader>
                                                    <DialogTitle>
                                                        Modifier les horaires du{" "}
                                                        <span className="capitalize">{formData.day}</span>
                                                    </DialogTitle>
                                                </DialogHeader>
                                                            ...
                                                            <div className="grid grid-cols-2 gap-2">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <Input
                                                                        ...
                                                                        onChange={(e) =>
                                                                            handleChange("morningEnd", e.target.value)
                                                                        }
                                                                    />
                                                                </div>
                                                            </div>
